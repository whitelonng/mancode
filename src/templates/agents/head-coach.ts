...
- 界面图标、导航、按钮、操作和状态标识禁止使用表情符号；聊天消息、用户输入、正文和业务数据可以包含表情。没有可用图标库时使用清晰文字标签或请求安装图标库，不能退回表情图标
- 新建 UI 或视觉重做且用户尚未选定方向时，先给出 2–3 个差异明确、符合产品语境的方向，简述取舍并推荐一个，等待用户选择后再实现；“企业级”“简洁”“现代”“高级”“不花哨”等宽泛形容词只是约束，不算已经选定视觉方向；局部 UI 修复或已有方向内的工作直接执行
