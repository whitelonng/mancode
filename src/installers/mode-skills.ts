...
export const KIMI_SKILL_MANAGED_MARKER =
  '<!-- Managed by mancode:kimi-skill. Do not edit this file manually. -->';
...
/**
 * Markers that identify a skill file as mancode-managed. Codex, ZCode, and
 * Kimi Code share the .agents/skills/ directory (all follow the agentskills.io
 * standard), so any marker means the file is safe to overwrite on reinstall.
 */
...
/**
 * Write mode skill files for Kimi Code (.agents/skills/<name>/SKILL.md).
 * Kimi Code scans .agents/skills/ alongside Codex and ZCode (agentskills.io
 * standard). Skill content is identical; only the managed marker and the
 * invocation prefix (`/skill:<name>`) differ.
 */
export async function installKimiSkills(
  projectRoot: string,
  minimal: boolean,
): Promise<void> {
  if (minimal) {
...
/**
 * Write command files for Qoder (.qoder/commands/<name>.md).
 * Qoder IDE and Qoder CLI both load project commands from .qoder/commands/;
 * frontmatter requires `name` and `description`, invoked with `/<name>`.
 */
const QODER_HOST_SCAN_GUIDANCE = [
...
export async function installQoderCommands(
  projectRoot: string,
  minimal: boolean,
): Promise<void> {
  if (minimal) {
  ...
  const commandsDir = path.join(projectRoot, '.qoder', 'commands');
  await mkdir(commandsDir, { recursive: true });
  for (const mode of MODE_NAMES) {
    const meta = MODE_META[mode];
    const content = [
...
export async function removeCodexSkills(projectRoot: string): Promise<void> {
  ...
  // Codex, ZCode, and Kimi Code share this directory. Preserve it while any
  // other non-minimal adapter still needs the same generated skills; otherwise
  // any platform marker is safe to remove because the shared content is identical.
  if (
    !(await anyPlatformNeedsSharedSkills(projectRoot, ['zcode', 'kimi-code']))
  ) {
...
export async function removeZcodeSkills(projectRoot: string): Promise<void> {
  // Current path — ZCode shares .agents/skills/ with Codex and Kimi Code.
  if (
    !(await anyPlatformNeedsSharedSkills(projectRoot, ['codex', 'kimi-code']))
  ) {
    await cleanManagedSkills(
      path.join(projectRoot, '.agents', 'skills'),
      MANCODE_AGENT_SKILL_MARKERS,
...
export async function removeKimiSkills(projectRoot: string): Promise<void> {
  // Kimi Code shares .agents/skills/ with Codex and ZCode (agentskills.io).
...
export async function removeDshSkills(projectRoot: string): Promise<void> {
  await cleanManagedSkills(
...
export async function removeQoderCommands(projectRoot: string): Promise<void> {
  for (const mode of MODE_NAMES) {
...
async function anyPlatformNeedsSharedSkills(
  projectRoot: string,
  platforms: readonly ('codex' | 'zcode' | 'kimi-code')[],
): Promise<boolean> {
...
async function platformNeedsSharedSkills(
  projectRoot: string,
  platform: 'codex' | 'zcode' | 'kimi-code',
): Promise<boolean> {
  try {
...
const MODE_META: Record<ModeName, ModeMeta> = {
  ...
  man: {
    ...
    workflow: [
      ...
      '',
      'Step 1 — Scout: run a bounded, read-only decision-impact discovery; test the user premise against repository evidence, inspect the end-to-end user goal and common domain failure/edge paths, and retain at most three material findings with stable IDs F-1 through F-3 and type premise, scope, technical, risk, or acceptance. Mark each repository_fact or domain_hypothesis; an unverified domain hypothesis becomes a focused question, never a fact. Discovery produces evidence and recommendations, never execution authority. Write `scout-report.md` with the profile, implementation, dependencies, risks, unknowns, and—when verifiable—Current Behavior Evidence, Candidate Semantic Owner with confidence/conflict, Source of Truth split into authority/readers/writer/derived copies, and Historical / Compatibility Impact for existing workflows, legacy fixtures, transport, migration, and rollback. Never invent fields to fill the template.',
      'Step 2 — Clarify by readiness, not a fixed round or question count. Ask every decision-changing question that cannot be resolved from project facts; use as many batches as needed without repeating confirmed answers. Classify unknowns as blocking, recommendable, or defaultable. When a good approach exists, present 2–3 options and make a clear recommendation. Give every F-ID one type-directed disposition: blocking findings stay in blockingUnknowns; accepted scope or behavior enters confirmedScope plus matching acceptanceCriteria; accepted technical choices enter technicalDecisions; only explicitly excluded behavior enters excludedScope; low-impact reversible details may enter defaults. An unaccepted proposal remains unauthorized and is not copied into every field. Create structured input with non-empty scope, exclusions, technical decisions, defaults, blocking unknowns, stable acceptance IDs, and explicit confirmed/defaulted/not-applicable coverage for platform, core scope, stack, data, performance, compatibility, and security. Run `mancode workflow requirements <taskId> finalize --file <input.json>`; only advance to Step 3 when the CLI returns ready.',
      'Step 3 — Plan Coach first returns READY_FOR_PLAN or NEEDS_CLARIFICATION. Never hide blocking gaps inside assumptions. Every option must solve the same goal, acceptance boundary, and scope; name its complexity bearer and observable cost; return exactly one recommendation with rejection reasons and stop conditions. A simple task may have one real direction and must not manufacture alternatives. When ready, write `plan.md` with confirmed scope, technical choices and reasons, reuse, exclusions, behavior, risks/rollback, completion criteria, defaults, and real validation. Add a user-visible implementation scope with non-empty repo-relative include, exclude, and module labels; include is the file-write upper bound, exclude wins, and modules do not authorize files. For cross-platform entry differences, unclear owner/source of truth, contract/state changes, cross-workflow/team/transport work, or multi-version compatibility, add an optional Domain Matrix inside plan.md; it is not authority. First plan is v1; revisions use `--plan-version <current+1>`.',
  ...
  manteam: {
    ...
    workflow: [
      ...
      '',
      'Run the same bounded Scout discovery as /man with stable F-IDs and typed findings. Discovery produces evidence and recommendations, never execution authority.',
      'Use the same type-directed disposition as /man, bind the confirmed plan to a user-visible implementation scope, and treat that scope as the file-write upper bound for claims, implementation, and review.',
      'After explicit user approval, a file-boundary-only adjustment may use `workflow scope change`; it versions plan authority, stales prior evidence, and reissues compatible claims. A behavior or acceptance change still requires reframe.',
      '',
