...
/**
 * Mode skill names — used for .codex/skills/, .zcode/skills/,
 ...
 *
 * Shared across Codex (.codex/skills/), ZCode (.zcode/skills/),
...
/**
 * Write mode skill files for Codex (.codex/skills/<name>/SKILL.md).
 */
...
/**
 * Write mode skill files for ZCode (.zcode/skills/<name>/SKILL.md).
 */
export async function installZcodeSkills(
  ...
  const skillsDir = path.join(projectRoot, '.zcode', 'skills');
  for (const mode of MODE_NAMES) {
    const modeDir = path.join(skillsDir, mode);
    await mkdir(modeDir, { recursive: true });
    const meta = MODE_META[mode];
