...
describe('mancode init', () => {
  ...
  it('force reinstalls every configured platform and migrates legacy Codex skills', async () => {
    await init(dir);
    const configPath = path.join(dir, '.mancode', 'config.json');
    const config = JSON.parse(await readFile(configPath, 'utf-8'));
    config.platforms = ['claude-code', 'codex'];
    config.platformOptions.codex = { minimal: false };
    await writeFile(configPath, `${JSON.stringify(config, null, 2)}\n`);
    const legacySkillDir = path.join(dir, '.agents', 'skills', 'mamba');
    ...
    await expect(
      readFile(path.join(legacySkillDir, 'SKILL.md'), 'utf-8'),
    ).rejects.toThrow();
  });

  it('recovers managed platforms when force repairing corrupt config', async () => {
    await init(dir, { platform: 'all' });
    const configPath = path.join(dir, '.mancode', 'config.json');
    await writeFile(configPath, '{ invalid config\n');

    expect(await init(dir, { force: true })).toBe(EXIT_OK);
    const repaired = JSON.parse(await readFile(configPath, 'utf-8'));
    expect(repaired.platforms).toEqual([
      'claude-code',
      'cursor',
      'codex',
      'copilot',
