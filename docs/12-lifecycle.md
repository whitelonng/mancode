...
2. 读取项目状态（tech stack, mode, team status）
...
echo "project_type: React + TypeScript"
echo "ui_library: shadcn/ui"
...
**职责**：
1. 在 solo 模式下追问 3 个问题
2. 读取审美 token（如果是前端任务）
3. 提醒 YAGNI 原则
...
│  User Prompt                        │
├─────────────────────────────────────┤
│  UserPromptSubmit Hook 注入         │  ← 追问 3 问、审美 token
├─────────────────────────────────────┤
│  System Prompt (SessionStart Hook) │  ← 模式、项目状态
├─────────────────────────────────────┤
│  SKILL.md (solo / man8 / man...)   │  ← 模式规则
├─────────────────────────────────────┤
│  Claude 基础能力                     │
└─────────────────────────────────────┘
```
...
| `currentMode` | string | 当前模式（solo / man8 / man / manteam / manps）|
| `lastMode` | string | 上一次模式 |
| `currentTask` | string \| null | 当前任务（如在 /man 流程中）|
| `skippedSteps` | string[] | 跳过的步骤（/man 流程中用户选择跳过的步骤）|
| `techStack` | string | 技术栈（React + TypeScript） |
| `uiLibrary` | string | UI 库（shadcn/ui / MUI / Ant Design / none）|
