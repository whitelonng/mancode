...
基线：`78d7e8ba64a236762da750e161b788a4cf1c813d`。本地环境：macOS arm64、Node 25.9.0，原生后端 `@napi-rs/keyring` 2.1.0。使用合成数据，没有生产凭据、npm 发布或远端变更。最终全量检查和提交状态见绑定计划的交付记录。
...
| S03 | 成功与密钥丢失：真实 Keychain 读写/删除及固定错误，丢失后零业务请求。服务锁定/用户拒绝通过 KeyProvider 错误注入验证失败关闭；未锁定用户登录钥匙串或实测人工拒绝弹窗。 |
| S04–S05 | 通过：密文/tag/nonce/entryId 篡改拒绝、nonce/revision 更新、旧批准失效。 |
| S06 | 通过：竞争写入拒绝；实际 EACCES 保留旧保险箱；目录缓存失败后已提交权威保持完整；中断临时文件不被当作权威。未做物理断电试验。 |
...
| S25 | 通过：真实 Codex 0.153.4 和 Claude Code 2.1.142 调用相同受审查 Ticket API 执行器，各一条业务请求。Codex 默认沙箱拒绝在前，精确命令的正常审批成功在后，原失败证据保留。 |
