...
本地运行时锁以完整 owner 文件的独占硬链接发布；发布前中断只留下未占锁的临时文件，发布后仍须确认进程死亡且租约过期才能回收。文件系统不支持硬链接时拒绝获取，不退回存在初始化空窗的目录占位方式。新运行时可读旧目录锁；旧运行时遇到新文件锁会拒绝操作，不保证新旧版本混用。无法确认归属的历史空目录锁不按超时猜测删除。Windows CI 直接运行锁发布、强杀恢复及旧格式兼容测试。
...
verification policy 1 保持流程指导；新建 man --delivery 任务可用 --execution-policy 显式启用 policy 2，将声明的检查、TDD 场景、修复预算和远端 CI 条件纳入完成门。该门禁不拦截宿主的任意文件编辑。详见 [工作流执行门禁](workflows.md#可选执行门禁)。
